...
# Perform simplified hierarchical clustering heatmap with selected features
# Select a few features for dendrogram heatmap ('alcohol', 'malic_acid', 'ash')
selected_features = wine_df[['alcohol', 'malic_acid', 'ash']]

# Standardize the selected features
X_selected_scaled = scaler.fit_transform(selected_features)

# Perform hierarchical clustering on selected features
row_clusters = linkage(X_selected_scaled, method='complete', metric='euclidean')

# Create dendrogram on the left
fig = plt.figure(figsize=(8, 8), facecolor='white')
axd = fig.add_axes([0.09, 0.1, 0.2, 0.6])

# Dendrogram with the hierarchical clustering results
row_dendr = dendrogram(row_clusters, orientation='left')

# Reorder the data based on the dendrogram clustering
df_rowclust = selected_features.iloc[row_dendr['leaves'][::-1]]

# Remove x and y ticks for the dendrogram
axd.set_xticks([])
axd.set_yticks([])
