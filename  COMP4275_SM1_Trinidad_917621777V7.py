...
### 2. DBSCAN clustering ###
dbscan = DBSCAN(eps=0.0001, min_samples=1000)
...
# plot row dendrogram and heatmap with label adjustments
fig = plt.figure(figsize=(12, 10), facecolor='white')  # Increase figure size for spacing
axd = fig.add_axes([0.09, 0.1, 0.2, 0.6])

# plot row dendrogram
row_dendr = dendrogram(linked_selected, labels=selected_features, orientation='left')

# reorder data with respect to clustering
df_rowclust = pd.DataFrame(X_selected, columns=selected_features).iloc[row_dendr['leaves'][::-1]]
...
# plot heatmap
axm = fig.add_axes([0.23, 0.1, 0.6, 0.6])  # Adjusted for larger heatmap
cax = axm.matshow(df_rowclust, interpolation='nearest', cmap='hot_r')

# Increase space between x-tick and y-tick labels
fig.colorbar(cax)
axm.set_xticks(np.arange(len(df_rowclust.columns)))  # Set number of ticks
axm.set_xticklabels(df_rowclust.columns, rotation=45, ha='right', fontsize=10)  # Adjust rotation and fontsize

axm.set_yticks(np.arange(len(df_rowclust.index)))  # Set number of ticks
axm.set_yticklabels(df_rowclust.index, fontsize=10)  # Adjust fontsize

# Increase padding between tick labels and plot using tick_params
axm.tick_params(axis='x', pad=10)  # Increase space for x-tick labels
axm.tick_params(axis='y', pad=10)  # Increase space for y-tick labels
